...
async function importSeparatedStems(
    ...
    for (const stem of stems) {
        ...
        // Create AudioData with proper SharedArrayBuffer frames
        const audioDataForStorage = AudioData.create(
            stem.audio.sampleRate,
            stem.audio.numberOfFrames,
            stem.audio.numberOfChannels
        )

        // Copy data into the shared buffers
        for (let ch = 0; ch < stem.audio.numberOfChannels; ch++) {
            const sourceFrame = stem.audio.frames[ch]
            const destFrame = audioDataForStorage.frames[ch]
            for (let i = 0; i < stem.audio.numberOfFrames; i++) {
